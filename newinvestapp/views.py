...
def signup(request):
	...
	if form.is_valid():
		...
		template= render_to_string('newinvestapp/welcomeEmail.html', {'name':username})
		email_message= EmailMessage(
			'Welcome on board to Asset Fussion!',
			template,
			settings.EMAIL_HOST_USER,
			[email],
...
def dashboard(request):
	...
	return render(request, 'newinvestapp/dashboard.html', context)
...
def deposit(request):
	if request.method=='POST':
		...
		if price_amount and pay_currency:
			...
			headers={'x-api-key':'RGNPW7P-NNBMT95-P20CXAQ-YZRKSCG', 'Content-Type': 'application/json'}
			...
			try:
				send_mail(client_username, "A client with username: {} wants to make a deposit of {}".format(client_username, price_amount),client_email, ['support@assetfussion.org'])
			except BadHeaderError:
				pass
			return redirect(generated_link)
	context={}
	return render(request, 'newinvestapp/deposit.html')
